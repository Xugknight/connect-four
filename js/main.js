...
let board; // 2D Array / 1/-1 -> player value; null -> cell is empty
...
// The init function's purpose is to intialize 
...
function init() {
    ...
    winner = null;
    turn = 1; // Can we make this random?
    render();
};
...
function renderBoard() {
    board.forEach((colArr, colIdx) => { // column array and column id
        colArr.forEach((cellVal, rowIdx) => { // cell value and row id
            const cellEl = document.getElementById(`c${colIdx}r${rowIdx}`);
            cellEl.style.backgroundColor = COLORS[cellVal];
            // cellEl.style.backgroundColor = 'red'; just testing
        });
    });
};
